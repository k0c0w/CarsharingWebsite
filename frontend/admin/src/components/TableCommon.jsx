...
export const TableSearchField = ({attrs, defaultAttrName, data, setData, customSearchFunc}) => {
    const theme = useTheme();
    const color = tokens(theme.palette.mode);
    ...
    <div style={{ marginTop: '70px' }}>
        <StyledTextField ref={serachFieldRef}
            style={{ width: '250px' }}
            type={'search'}
            helperText={`Фильтровать по ${getAttr(attrs, attr)?.labelHelper?.toLowerCase() ?? attr}`}>
        </StyledTextField>  
        <StyledTextField
            style={{ width: '170px' }}
            select
        ...
        <Button
            ...
            variant={'contained'}
            onClick = {()=>{
                setData(searchFunc(data, serachFieldRef.current.childNodes[0].childNodes[0].value, attr))}}
        >
